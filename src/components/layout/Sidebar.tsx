...
/** Big, non-clickable section header row (same size as top-level items) */
...
const Sidebar: React.FC = () => {
  ...
  const mAgentStatus = useMatch('/clients/:clientId/agent-status/*');
      ...
      <div className="flex-1 p-4 space-y-6">
        ...
        <div>
          ...
          <div className="space-y-1">
            <NavLink to="/clients" end className={({ isActive }) => itemClass(isActive)}>
            ...
            {isInClientSection && !isCollapsed && (
              <div className="space-y-1 mt-2">
                ...
                <NavLink to={`${base}/agent-status`} className={() => itemClass(!!mAgentStatus, true)}>
                  <Activity className="w-4 h-4 shrink-0" />
                ...
                <div className="mt-6 pt-3 border-t border-white/10 space-y-2">
                  <div className={sectionHeaderClass()}>
                    <ShieldX className="w-5 h-5 shrink-0 text-[#E9EEF6]" />
                    <span className="truncate font-medium">Ignored Checks</span>
                  </div>
        ...
        {/* Resources – styled like a big section row */}
        <div className="mt-6 pt-3 border-t border-white/10 space-y-2">
          {!isCollapsed && (
            <div className={sectionHeaderClass()}>
              <BookOpen className="w-5 h-5 shrink-0 text-[#E9EEF6]" />
              <span className="truncate font-medium">Resources</span>
            </div>
          )}
          ...
          <div className="space-y-1">
            ...
            <div className="mt-6 pt-3 border-t border-white/10 space-y-2">
              <div className={sectionHeaderClass()}>
                <ClipboardList className="w-5 h-5 shrink-0 text-[#E9EEF6]" />
                <span className="truncate font-medium">Infrastructure Assessment</span>
              </div>

              <div className="space-y-1">
                <NavLink to="/resources/onboarding" end className={({ isActive }) => itemClass(isActive, true)}>
                  <ClipboardList className="w-4 h-4 shrink-0" />
                  <span className="truncate">Onboarding Questionnaire</span>
                </NavLink>
                <NavLink to="/infrastructure/asset-comparator" end className={({ isActive }) => itemClass(isActive, true)}>
                  <BarChart3 className="w-4 h-4 shrink-0" />
                  <span className="truncate">Asset Comparator</span>
                </NavLink>
                <NavLink to="/unknown-tracer" end className={({ isActive }) => itemClass(isActive, true)}>
                  <AlertTriangle className="w-4 h-4 shrink-0" />
                  <span className="truncate">Unknown Tracer</span>
                </NavLink>
              </div>
            </div>
          </div>
