...
import { Plus, Trash2, MonitorX, Search, Filter, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
...
interface IgnoredDevicesProps {
  client: ClientDetails;
}

const IgnoredDevices: React.FC<IgnoredDevicesProps> = ({ client }) => {
  const [ignoredDevices, setIgnoredDevices] = useState<IgnoredDevice[]>([
    {
      id: 'device-1',
      deviceName: 'PRINTER-HP-01',
      ipAddress: '192.168.1.150',
      macAddress: '00:1B:44:11:3A:B7',
      reason: 'Network printer - not security relevant',
      addedBy: 'Omar Sleem',
    ...
    {
      id: 'device-2',
      deviceName: 'IOT-SENSOR-TEMP',
      ipAddress: '192.168.1.201',
      macAddress: '02:42:AC:11:00:02',
      reason: 'Temperature sensor - limited security impact',
    ...
    {
      id: 'device-3',
      deviceName: 'GUEST-WIFI-AP',
      ipAddress: '192.168.2.1',
      reason: 'Guest network access point - isolated network',
      addedBy: 'Omar Sleem',
  ...
  const filteredDevices = ignoredDevices.filter(item => {
  ...
  const addIgnoredDevice = () => {
    if (!newDevice.deviceName.trim() || !newDevice.ipAddress.trim() || !newDevice.reason.trim()) return;

    const newItem: IgnoredDevice = {
      id: `device-${Date.now()}`,
    ...
    setIgnoredDevices(prev => [...prev, newItem]);
    setNewDevice({ deviceName: '', ipAddress: '', macAddress: '', reason: '' });
    setShowAddForm(false);
  };

  const toggleDeviceStatus = (id: string) => {
    setIgnoredDevices(prev => prev.map(item => 
      item.id === id ? { ...item, isActive: !item.isActive } : item
    ));
  };

  const deleteDevice = (id: string) => {
    if (confirm('Are you sure you want to delete this ignored device?')) {
      setIgnoredDevices(prev => prev.filter(item => item.id !== id));
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white flex items-center">
            <MonitorX className="h-6 w-6 mr-3 text-[#14D8C4]" />
            Ignored Devices - {client.companyName}
          </h1>
          <p className="text-slate-400 mt-2">Manage devices that are excluded from security monitoring</p>
        </div>
        <button
      ...
      <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700/50">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0 lg:space-x-6">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search devices, IPs, or reasons..."
              value={searchQuery}
      ...
      {showAddForm && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-[#111726]/95 backdrop-blur-md border border-white/8 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.45)] p-6">
            <h2 className="text-xl font-semibold text-white mb-6">Add Ignored Device</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">MAC Address (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setNewDevice(prev => ({ ...prev, reason: e.target.value }))}
                  placeholder="Explain why this device should be ignored..."
      ...
      <div className="bg-[#0B1220]/80 backdrop-blur-md border border-white/10 rounded-[18px] shadow-[0_8px_32px_rgba(0,0,0,0.35)] overflow-hidden">
        <div className="p-6">
          <h2 className="text-xl font-semibold text-white mb-6">
            Ignored Devices ({filteredDevices.length})
          </h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  <th className="text-left py-3 px-4 text-sm font-medium text-slate-400">Device Name</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-slate-400">IP Address</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-slate-400">MAC Address</th>
          ...
          {filteredDevices.length === 0 && (
            <div className="text-center py-8">
              <MonitorX className="h-12 w-12 text-slate-400 mx-auto mb-4" />
              <p className="text-slate-400">No ignored devices found</p>
            </div>
          )}
        </div>
      </div>
